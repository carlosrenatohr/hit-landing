import { Package, Search, Truck } from "lucide-preact";
...
export const TrackingPortal = () => {
  ...
  const trackingInfo = {
    number: trackingNumber || "HIT20240720-001",
    status: "En Tránsito",
    ...
    <div className="container mx-auto px-4 md:px-6 py-20">
      <div className="text-center mb-12">
        ...
        <p className="text-xl text-neutral-text dark:text-gray-300 max-w-3xl mx-auto">
          Ingresa tu número de rastreo para obtener actualizaciones en tiempo
          real sobre el estado y ubicación de tu envío.
        </p>
      </div>
      ...
      <div className="max-w-2xl mx-auto mb-16">
        <div className="bg-white dark:bg-secondary-light p-8 rounded-lg shadow-lg border border-primary/10">
          <form onSubmit={handleTrack}>
            ...
            <button
              type="submit"
              className="w-full mt-4 bg-accent-blue text-white py-4 text-lg rounded-md font-bold hover:bg-blue-600 transition-all"
            >
              Rastrear Paquete
            </button>
          </form>
          <p className="text-sm text-neutral-text dark:text-gray-400 mt-4 text-center">
            Los números de rastreo típicamente comienzan con "HIT" seguido de la
            fecha y un número de secuencia.
          </p>
        </div>
      </div>
          ...
          <div className="bg-white dark:bg-secondary-light p-8 rounded-lg shadow-lg mb-8 border border-primary/10">
            <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-8">
              <div>
                <h2 className="text-2xl font-bold text-secondary dark:text-white mb-2">
                  Número de Rastreo: {trackingInfo.number}
                </h2>
                <p className="text-neutral-text dark:text-gray-300">
                  Fecha de Envío: {trackingInfo.shipDate}
                </p>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              <div className="bg-neutral-bg dark:bg-secondary p-4 rounded-md">
                <p className="text-sm font-medium text-neutral-text dark:text-gray-400 mb-1">
                  Entrega Estimada
                </p>
              ...
              {/* Other details simplified/omitted for logic focus */}
            </div>
                ...
                {trackingInfo.events.map((event, index) => (
                  <div key={index} className="flex">
                    <div className="flex-shrink-0 relative z-10">
                      ...
                      >
                        {/* Icons would need proper rendering here */}
                        <Package
                          class={`h-6 w-6 ${index === 0 ? "text-secondary" : "text-gray-500"}`}
                        />
                      </div>
                    </div>
                    <div className="ml-4">
                      ...
                      <div className="text-sm text-neutral-text dark:text-gray-400 mt-1">
                        {event.date}
                      </div>
